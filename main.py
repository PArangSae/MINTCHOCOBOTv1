...
async def on_message(message):
    ...
    if len(messageattachments) > 0:
        for attachment in messageattachments:
            if attachment.filename.endswith(".dll"):
                ...
                await message.delete()
                await message.channel.send(message.author.mention+" :no_entry: DLL이나 EXE 파일은 무단 배포를 막기 위해 올릴 수 없어요!")
            elif attachment.filename.endswith('.jfif'):
                await message.delete()
                await message.channel.send(message.author.mention+" :no_entry: JFIF 파일은 사용자들의 편의상 올릴 수 없어요!")
            else:
                return None
    ...
    if messageContent.startswith("상자 ") == True:
        ...
        if messageContent[3:] == "안녕":
        ...
        elif messageContent[3:] == "파랑새":
            await message.channel.send("잡덱님, 파이어리자드님의 도움을 받아 저를 창조하신 저의 개발자이시죠!")
            ...
            if messageContent[3:] in minask:
                ...
                tosendd = tosendd.rstrip("'")
                tosendd = tosendd.replace("', '", " ")
                await message.channel.send(tosendd)
            else:
                olo = random.randint(0,6)
                ne = messageContent[3:]
                if olo == 0:
